...
import { loadGlobalConfig, getProjectPaths, loadProjectConfig, PACKAGE_ROOT } from './config.js';
...
import { runUpdateMalwareHashes } from './update-malware-hashes.js';
...
function printHelp() {
  console.log(`npm-verify-guard v${PACKAGE_VERSION} - npm vulnerability and malware verifier
...
Usage:
  ...
  npm-verify update-hashes [--dry-run] [--no-merge] [--concurrency N]
...
export async function runCli(argv) {
  ...
  switch (args.command) {
    ...
    case 'update-hashes': {
      const { result, outputPath, dryRun } = await runUpdateMalwareHashes({
        dryRun: args.flags.has('dry-run'),
        merge: !args.flags.has('no-merge'),
        concurrency: Number.parseInt(args.options.concurrency ?? '4', 10),
        onProgress: ({ level, message }) => {
          if (level === 'warn') {
            console.warn(`npm-verify: ${message}`);
          }
        },
      });

      console.log(`Hashes collected: ${result.hashes.length}`);
      console.log(`Entries with metadata: ${result.entries.length}`);
      if (result.stats?.osvNpm) {
        console.log(
          `OSV npm: ${result.stats.osvNpm.osvReports} reports, ${result.stats.osvNpm.packageVersions} versions checked`,
        );
      }

...
import { tokenizeText, tokensToVector } from './tokenizer.js';
...
export async function classifyNewsText(text, options = {}) {
  ...
  const tokens = tokenizeText(text, keywords);
  const newsModel = await loadNewsMlpModel();
  const vector = tokensToVector(tokens, newsModel.vocabulary);
  ...
  if (options.deepScan && mlConfig.useOnnx !== false) {
    ...
    if (onnx.available) {
      try {
        const result = await classifyWithOnnx(onnx.session, text, 'news');
        return mapNewsProbability(result.probability, 3, 'ml-onnx-distilbert');
      } catch {
        // fallback below
      }
    }
  ...
  return mapNewsProbability(layer2.probability, options.deepScan ? 3 : 2, 'ml-news-mlp');
}

function mapNewsProbability(probability, layer, rule) {
  if (probability >= 0.8) {
